...
func _ready():
	...
	get_node("AnimationPlayer").play("flicker",-1,rand_range( 0.9, 1.1 ))
	if (randi()%5 == 1):
...
func handle_collision():
	...
	if (enemy_in_range.size() > 0):
		if (tick <= 0.0):
			experience += enemy_in_range[0].damage(my_level)
...
func get_goal():
	if (insane_time > 0.0):
		return insane_goal
	if ( (last_tick_pos - get_translation()).length() < 0.1 and tick <= 0.0):
		randomize()
